...
public class MOHRadiantMAPWriter {
	...
	public void write() throws java.io.IOException, java.lang.InterruptedException {
		...
		if(BSPVersion==BSP.TYPE_NIGHTFIRE) {
				...
				for(int j=0;j<data.getElement(i).getNumBrushes();j++) {
					if(data.getElement(i).getBrushes()[j].isWaterBrush()) {
						...
						for(int k=0;k<currentBrush.getNumSides();k++) {
							...
							if(currentBrushSide.getPlane().getNormal().equals(Vector3D.UP) || currentBrushSide.getPlane().getNormal().equals(Vector3D.UP.negate())) {
								currentBrushSide.setTexture("test/omaha_pjspick5");
							} else {
								currentBrushSide.setTexture("common/nodraw");
							}
						}
