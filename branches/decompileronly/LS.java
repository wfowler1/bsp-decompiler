			...
			try {
				...
				if(in == 1347633737) { // 1347633737 reads in ASCII as "IBSP"
					...
					if(in == 892416050) { // 892416050 reads in ASCII as "2015," the game studio which developed MoHAA
						BSP.read(input);
						version=46; // The BSP format is almost identical to the Quake III format. But you'll never find the version number in the BSP file.
					} else {
						if(in == 1095516485) { // 1095516485 reads in ASCII as "EALA"
							BSP.read(input);
							version=46; // This is just an MoHAA expansion pack.
						} else {
				...
				switch(version) {
					...
					case 42: // JBN
						numLumps=18;
						break;
					case 46: // Quake 3/derivative (MoHAA, etc.)
						numLumps=17;
						break;
