...
public class DecompilerThread implements Runnable {
	...
	public DecompilerThread(File BSP, boolean vertexDecomp, boolean correctPlaneFlip, boolean calcVerts, boolean roundNums, boolean toVMF, int jobnum, int threadnum) {
	...
	public void run() {
			...
			if(doomMap!=null) { // If this is a Doom map extracted from a WAD
				...
				switch(reader.getVersion()) {
					case 38:
						...
						decompiler = new Decompiler(reader.BSP38, vertexDecomp, correctPlaneFlip, calcVerts, roundNums, toVMF, jobnum);
						decompiler.decompile();
						break;
					case 42:
						Window.setProgress(jobnum, 0, reader.BSP42.getBrushes().getNumElements()+reader.BSP42.getEntities().getNumElements(), "Decompiling...");
						decompiler = new Decompiler(reader.BSP42, vertexDecomp, correctPlaneFlip, calcVerts, roundNums, toVMF, jobnum);
						decompiler.decompile();
						break;
					case 46:
						Window.setProgress(jobnum, 0, reader.BSP46.getBrushes().getNumElements()+reader.BSP46.getEntities().getNumElements(), "Decompiling...");
						decompiler = new Decompiler(reader.BSP46, vertexDecomp, correctPlaneFlip, calcVerts, roundNums, toVMF, jobnum);
						decompiler.decompile();
						break;
				}
			}
		...
		} catch (java./*lang.*/io.IOException e) {
