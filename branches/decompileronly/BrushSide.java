...
public class BrushSide extends LumpObject {
	...
	// CONSTRUCTORS
	public BrushSide(LumpObject in, int type, boolean isVindictus) {
		super(in.getData());
		new BrushSide(in.getData(), type, isVindictus);
	}
	
	public BrushSide(byte[] data, int type, boolean isVindictus) {
		super(data);
		if(!isVindictus) {
			switch(type) {
